...
LAN内自動同期つき組み込みテーブルDBです。Node.js/Bunアプリケーションに組み込んで使います。
...
- グループ管理： `groupKey`で同期グループを区別し、異なるグループのpeerは無視する（`groupKey`未指定時はローカル専用）
- 単一バイナリ対応： native addon不使用、Bun `bun build --compile`で動作

## クイックスタート

```ts
import { createLanSyncDb } from 'mesh-db';
...
```ts
type Options = {
  groupKey?: string; // 同期グループ識別キー（省略時はLAN自動発見なし）
  dir: string; // 永続化ディレクトリ
  tables: Record<string, { primaryKey: string }>;
  sync?: {
    enabled?: boolean; // 共有機能のON/OFF（デフォルト: groupKeyあり=true, なし=false）
    discovery?: boolean; // mDNS自動発見（デフォルト: true、groupKeyなしでは常にfalse）
    transport?: boolean; // WebSocketサーバー起動（デフォルト: true）
    port?: number; // WebSocketサーバーポート（0は空きポート、指定なければ0扱い）
    serviceType?: string; // mDNS service type（デフォルト: '_mesh-db._tcp'）
    peerId?: string; // 永続peerIdの明示設定（指定時はpeer-idファイルに保存される）
  };
  seed?: Record<string, Record<string, unknown>[]>; // 初期データ（store.jsonが空のときのみ投入）
};
```
...
type LanSyncDb = {
  ...
  groupId: string | undefined; // groupKeyのSHA-256ハッシュ（groupKeyなし時はundefined）
  port: number | undefined; // WebSocketサーバーポート（transport:false/enabled:false時はundefined）
  table(name: string): TableHandle; // テーブルハンドルの取得
  connect(host: string, port: number): Promise<void>; // 他peerへ手動接続
...
`groupKey`と`sync.enabled`の組み合わせで動作モードが変わります。

| `groupKey` | `sync.enabled` | 挙動                        | discovery | direct connect |
| ---------- | -------------- | --------------------------- | --------- | -------------- |
| あり       | 未指定         | 共有ON、同一groupと自動同期 | 可        | 可             |
| あり       | `true`         | 同上                        | 可        | 可             |
| あり       | `false`        | 共有完全OFF                 | なし      | 不可           |
| なし       | 未指定         | 共有OFF                     | なし      | 不可           |
| なし       | `true`         | 手動共有ON                  | なし      | 可             |
| なし       | `false`        | 共有完全OFF                 | なし      | 不可           |

- `connect()`は`sync.enabled`が`false`のとき`Sync is disabled`で失敗します
- `connect()`はtransport未起動のとき`Transport is disabled`で失敗します
...
| `GroupIdMismatchError`   | 異なる`groupKey`のpeer、または片側だけ`groupKey`があるpeerに接続しようとした |
...
`createLanSyncDb`に`seed`オプションを渡すと、DBが空の状態（`store.json`が存在しない、または空）のときだけ初期データを投入できます。既存データがある場合はseedをスキップするため、再起動時に意図せず二重投入されることはありません。
...
const db = await createLanSyncDb({
  ...
  seed: {
    ...
    posts: [{ id: 'p1', title: 'Hello, world!', author: 'alice' }],
  },
});
```

- seedは`upsert`と同じ動作で各行を挿入します
- 各レコードには必ずprimary keyを含めてください
- 存在しないテーブル名を指定すると`createLanSyncDb`がエラーで失敗します
